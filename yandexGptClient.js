...
// В отличие от GigaChat, здесь НЕ нужен OAuth-обмен и кастомный CA-сертификат:
// авторизация — статический API-ключ сервисного аккаунта, TLS — обычный публичный.
// Это заметно упрощает клиент и убирает целый класс операционных проблем.
//
...
//
// Переменные окружения (см. пошаговую инструкцию по настройке отдельно):
 ...
 *   Тот же формат {role, content}, что мы уже используем для промпта —
 *   здесь просто маппится в {role, text}, как того требует Yandex API.

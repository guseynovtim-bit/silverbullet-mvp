...
// Метрики с планом (факт + план)
const METRIC_FIELDS_WITH_PLAN = [
  { key: 'mau', label: 'MAU', hint: 'MAU × ARPU ≈ выручка квартала' },
  { key: 'arpu', label: 'ARPU' },
  { key: 'conversion_mau_share', label: 'Конверсия MAU, %' },
  { key: 'churn_rate', label: 'Отток, %' },
  { key: 'completed_tasks', label: 'Выполнено задач' },
];

// Метрики без плана (только факт)
const METRIC_FIELDS_NO_PLAN = [
  { key: 'team_size', label: 'Размер команды' },
  { key: 'planned_tasks', label: 'Запланировано задач' },
  { key: 'problematic_releases', label: 'Проблемные релизы' },
];
...
const buildInitialMetrics = () => {
  const metrics = {};
  METRIC_FIELDS_WITH_PLAN.forEach(({ key }) => {
    metrics[key] = { value: '', plan: '' };
  });
  METRIC_FIELDS_NO_PLAN.forEach(({ key }) => {
    metrics[key] = { value: '', plan: '' }; // plan держим пустой строкой, а не null,
    // чтобы инпут оставался controlled на всём протяжении жизни компонента
  });
  return metrics;
};
...
export default function FormQBR({ onSubmit, defaultQuarter = 'Q3 2026', defaultTeam = 'Core Product Team' }) {
  ...
  const isValid = () => {
    // Пример базовой валидации: факт по каждой метрике с планом должен быть заполнен
    return METRIC_FIELDS_WITH_PLAN.every(({ key }) => data.metrics[key].value !== '');
  };
    ...
    >
      ...
      <div className="p-4 bg-gray-50 rounded-md border border-gray-200 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {METRIC_FIELDS_WITH_PLAN.map(renderMetricWithPlan)}
        </div>
      </div>
      ...
      <div className="p-4 bg-gray-50 rounded-md border border-gray-200 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {METRIC_FIELDS_NO_PLAN.map(renderMetricNoPlan)}
        </div>
      </div>
